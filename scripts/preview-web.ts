...
async function main(): Promise<void> {
  ...
  const { recordingPage, recordingsIndexPage } = pageModule;
  ...
  const [fixtureMeta, fixtureTranscript, fixtureMinutes] = await Promise.all([
  ...
  const server = app.listen(previewPort, '127.0.0.1', () => {
    ...
    console.log(
      'Rotas: / · /en · /docs · /en/docs · /demo · /en/demo · /preview/app · /preview/discord-demo · /preview/empty · /preview/live · /preview/audio-gone',
    );
  });
