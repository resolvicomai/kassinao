...
MINUTES_ENABLED=false`,
  ...
  const hostProxyNginx = codeBlock(
    'Nginx',
    `location / {
...
# KASSINAO_UID=${T('<ID livre entre 61000 e 61183>', '<unused ID from 61000 through 61183>')}
# KASSINAO_GID=${T('<ID livre entre 61000 e 61183>', '<unused ID from 61000 through 61183>')}
...
# ${T('Antes de continuar, defina UID e GID explicitamente após provar que não colidem com contas, grupos, subuids/subgids, processos, arquivos protegidos, containers vizinhos nem árvores anteriores do Kassinão. Não crie usuário ou grupo Linux para esse par.', 'Before continuing, set UID and GID explicitly after proving they do not collide with accounts, groups, subordinate IDs, processes, protected files, neighboring containers, or prior Kassinão trees. Do not create a Linux user or group for this pair.')}
...
sudoedit "$RELEASE_ROOT/.env"
# ${T('Defina os oito limites CORE/ROUTER/PUBLIC/TUNNEL de memória e CPU; a soma precisa preservar ao menos 25% da capacidade física para os vizinhos. Monitore I/O separadamente.', 'Set the eight CORE/ROUTER/PUBLIC/TUNNEL memory and CPU limits; their sum must preserve at least 25% of physical capacity for neighbors. Monitor I/O separately.')}
...
# KASSINAO_SHARED_APP_ENV_FILE=$DATA_ROOT/config/app.env
# KASSINAO_SHARED_TUNNEL_TOKEN_FILE=$DATA_ROOT/config/cloudflared-token
# ${T('Defina UID e GID agora, antes do downtime, após provar que não colidem com contas, grupos, subuids/subgids, processos, arquivos protegidos, containers vizinhos nem árvores anteriores do Kassinão. Não crie usuário ou grupo Linux para esse par.', 'Set UID and GID now, before downtime, after proving they do not collide with accounts, groups, subordinate IDs, processes, protected files, neighboring containers, or prior Kassinão trees. Do not create a Linux user or group for this pair.')}
...
# ${T('Depois desse ponto, o plaintext preservado é rollback de dados, não um rollback operacional pronto. O bundle traz a primitiva selada de topologia, mas ambiente de resgate e runbook ficam em um adapter root-only privado preparado antes da migração; sem ele, a recuperação é fix-forward pelo adapter shared.', 'After this point, the preserved plaintext is a data rollback, not a ready-to-use operational rollback. The bundle carries the sealed topology primitive, while rescue configuration and the runbook stay in a private root-only adapter prepared before migration; without it, recovery is fix-forward through the shared adapter.')}
...
sudo "$RELEASE_ROOT/scripts/audit-shared-vps-security.sh"`,
  ...
  const finalizeSharedMigration = codeBlock(
    T('Finalizar rollback de dados plaintext shared', 'Finalize the shared plaintext data rollback'),
...
<div class="docs-shell">
  ...
  <main class="docs-main" id="conteudo"><div class="docs-main-inner">
    ...
    <section class="doc-section" id="contexto" data-doc-section data-keywords="commitments combinados jira github follow notifications">
      <div class="section-head"><h2>${esc(T('Acompanhar o que ficou combinado', 'Follow meeting commitments'))}</h2><p>${esc(T('No app privado, confirme ações da ata, reúna menções de calls diferentes e vincule fontes exatas. Cada menção mantém sua reunião, acesso, histórico e estado.', 'In the private app, confirm minutes actions, link mentions from different calls, and attach exact sources. Each mention retains its meeting, access rules, history, and state.'))}</p></div>
      <p>${esc(T('Jira e GitHub exigem escopos definidos pelo operador, concessão com validade e credencial pessoal de leitura. O bot consulta a fonte exata, sem criar tickets, comentar ou fazer merge. Documentos são links manuais; seu conteúdo não é consultado.', 'Jira and GitHub require operator-defined scopes, an expiring reader grant, and personal read credentials. The bot reads the exact source without creating tickets, commenting, or merging. Documents are manual links; their contents are not read.'))}</p>
      <p>${esc(T('Seguir um combinado ou canal permite avisos privados de mudanças. É possível silenciar ou pausar. Eventos agendados do Discord permitem lembretes de preparação; sem evento, o bot não prevê a próxima call. Um PR integrado não comprova implantação; critérios de conclusão precisam ser escolhidos explicitamente.', 'Following a commitment or channel enables private change notices. You can mute or pause them. Discord scheduled events enable preparation reminders; without an event, the bot cannot predict the next call. A merged PR does not prove deployment; completion criteria must be selected explicitly.'))}</p>
    </section>
    ...
    <section class="doc-section" id="local" data-doc-section data-keywords="local source docker build compose localhost quickstart">
      ...
      <div class="callout"><strong>${esc(T('Quickstart exige Docker Engine 28.1.0+ e Docker Compose 2.36.0+.', 'Quickstart requires Docker Engine 28.1.0+ and Docker Compose 2.36.0+.'))}</strong><p>${esc(T('O Engine 28.1 é o primeiro que aplica o interface_name do Compose. O router depende dessas interfaces nomeadas e da seleção determinística de gateway; versões anteriores são recusadas antes de subir.', 'Engine 28.1 is the first release that applies Compose interface_name. The router depends on those named interfaces and deterministic gateway selection; older versions are rejected before startup.'))}</p></div>
      <div class="quick-layout"><ol class="quick-list"><li><strong>${esc(T('Crie um app Discord de teste', 'Create a test Discord app'))}</strong><span>${esc(T('Use uma guild separada e credenciais descartáveis.', 'Use a separate guild and disposable credentials.'))}</span></li><li><strong>${esc(T('Clone e construa', 'Clone and build'))}</strong><span>${esc(T('A imagem local recebe o nome kassinao-local:dev.', 'The local image is named kassinao-local:dev.'))}</span></li><li><strong>${esc(T('Configure o mínimo', 'Configure the minimum'))}</strong><span>${esc(T('Defina o app, localhost e a guild de teste; mantenha todo egress desligado.', 'Set the app, localhost, and test guild; keep all egress disabled.'))}</span></li><li><strong>${esc(T('Suba sem pull', 'Start without pulling'))}</strong><span>${esc(T('Compose usa exatamente a imagem que você acabou de construir.', 'Compose uses the exact image that you just built.'))}</span></li></ol><div class="code-stack">${localBuild}${localEnv}${localStart}</div></div>
      <div class="callout"><strong>${esc(T('O quickstart local não valida a operação de uma VPS.', 'The local quickstart does not validate VPS operations.'))}</strong><p>${esc(T('Mesmo no localhost, o router é o único processo publicado; core e processo público não expõem portas próprias. Produção exige domínio HTTPS próprio, política do operador, storage criptografado, bundle verificado e auditoria do host.', 'Even on localhost, the router is the only published process; the core and public process expose no host ports. Production requires your own HTTPS domain, an operator policy, encrypted storage, a verified bundle, and a host audit.'))}</p></div>
    </section>
    ...
    <section class="doc-section" id="producao" data-doc-section data-keywords="production release bundle ghcr digest attestation split public luks vps dedicated shared">
      ...
      <div class="callout"><strong>${esc(T('Proxy do host exige headers exatos.', 'A host proxy requires exact headers.'))}</strong><p>${esc(T('Use somente a porta loopback. Preserve Host, apague CF-Connecting-IP e sobrescreva X-Forwarded-For com o IP visto pelo próprio proxy. O router não aceita CF-Connecting-IP isolado e reconstrói host, protocolo e porta para o upstream.', 'Use only the loopback port. Preserve Host, delete CF-Connecting-IP, and overwrite X-Forwarded-For with the IP observed by the proxy itself. The router does not accept CF-Connecting-IP alone and rebuilds host, protocol, and port for the upstream.'))}</p></div>
      <div class="code-stack">${hostProxyNginx}</div>
      ...
      <div class="callout danger"><strong>${esc(T('Rollback de dados não é rollback operacional.', 'Data rollback is not operational rollback.'))}</strong><p>${esc(T('O plaintext preservado não deve ser religado manualmente. O bundle público inclui a primitiva selada de topologia, mas a reversão operacional só existe se um adapter root-only privado, com ambiente de resgate e runbook específicos do host, foi preparado e testado antes da migração. Sem ele, a recuperação é fix-forward pelo shared.', 'The preserved plaintext must not be restarted manually. The public bundle includes the sealed topology primitive, but operational reversal exists only if a private root-only adapter with host-specific rescue configuration and runbook was prepared and tested before migration. Without it, recovery is fix-forward through shared.'))}</p></div>
      <div class="code-stack">${productionDeployDedicated}${productionDeployShared}${productionMigrateShared}${rebootSharedStorage}</div>
      <div class="callout"><strong>${esc(T('Rollback não é backup.', 'Rollback is not a backup.'))}</strong><p>${esc(T('Um deploy saudável tenta apagar o snapshot imediatamente. Se essa limpeza ou o deploy falhar, o snapshot contém somente estado operacional e metadados de gravações, sem auth nem faixas de áudio, e o timer persistente do host impõe a remoção dentro da janela declarada mesmo sem outro deploy.', 'A healthy deployment attempts to delete its snapshot immediately. If that cleanup or the deployment fails, the snapshot contains only operational state and recording metadata, without auth or audio tracks, and the persistent host timer enforces removal within the declared window even if no later deployment runs.'))}</p></div>
    </section>
