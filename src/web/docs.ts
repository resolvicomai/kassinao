...
# ${T('Depois desse ponto, o plaintext preservado é rollback de dados, não rollback operacional. Não religue o Compose legado nem reinstale controles dedicated; a recuperação do serviço é fix-forward pelo adapter shared.', 'After this point, the preserved plaintext is a data rollback, not an operational rollback. Do not restart the legacy Compose stack or reinstall dedicated controls; service recovery is fix-forward through the shared adapter.')}
...
<div class="docs-shell">
  ...
  <main class="docs-main" id="conteudo"><div class="docs-main-inner">
    ...
    <section class="doc-section" id="producao" data-doc-section data-keywords="production release bundle ghcr digest attestation split public luks vps dedicated shared">
      ...
      <div class="callout danger"><strong>${esc(T('A migração não oferece rollback operacional.', 'The migration does not provide operational rollback.'))}</strong><p>${esc(T('O plaintext preservado é rollback de dados. Depois que o migrador conclui, não religue o Compose legado nem reinstale controles dedicated no host shared; a recuperação do serviço é fix-forward. Se reversão operacional for requisito, pare antes da migração e use um adapter de reversão separado e testado no host.', 'The preserved plaintext is a data rollback. After the migrator succeeds, do not restart the legacy Compose stack or reinstall dedicated controls on the shared host; service recovery is fix-forward. If operational reversal is required, stop before migration and use a separate, host-tested reversal adapter.'))}</p></div>
