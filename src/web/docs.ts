...
const ENV_GROUPS: EnvGroup[] = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        fallback: { pt: 'vazio', en: 'empty' },
        description: {
          pt: 'Use tunnel para subir o Cloudflare Tunnel e tunnel,autoheal para incluir o autoheal.',
          en: 'Use tunnel to start Cloudflare Tunnel and tunnel,autoheal to include autoheal.',
        },
      },
      {
        name: 'COOKIE_SECRET',
...
<script>try{var theme=localStorage.getItem('kassinao-docs-theme');if(theme==='light'||theme==='dark')document.documentElement.setAttribute('data-theme',theme)}catch(_){}</script>
    ...
    <div class="docs-main-inner">
      ...
      <section class="doc-section" id="privacidade" data-doc-section data-keywords="privacy permissions oauth access consent restricted channel retention delete fail closed prompt injection personal data">
        ...
        <div class="privacy-layout">
          <div class="privacy-rules">
            ...
            <article class="privacy-rule"><h3>${esc(T('Histórico de canal restrito', 'Restricted-channel history'))}</h3><p>${esc(T('Só abre para quem estava na call, mesmo mutado, quem iniciou ou um admin atual. Receber permissão depois não libera o passado.', 'Only call participants, including muted participants, the starter, or a current admin can open it. Later permission does not unlock the past.'))}</p></article>
      ...
      <section class="doc-section" id="problemas" data-doc-section data-keywords="troubleshooting error not online commands missing oauth callback tunnel transcript minutes mcp 404 denied disk audio expired">
        ...
        <div class="troubleshooting">
          ...
          <details class="trouble"><summary>${esc(T('Uma pessoa recebeu acesso negado', 'Someone received access denied'))}</summary><div class="trouble-body"><p>${esc(T('Confirme que ela continua no servidor. Em canal restrito, ela precisa ter estado na call, ter iniciado a gravação ou ser admin atual. Ganhar acesso ao canal depois não libera o histórico.', 'Confirm they are still in the server. For a restricted channel, they must have joined the call, started the recording, or be a current admin. Later channel access does not unlock history.'))}</p></div></details>
