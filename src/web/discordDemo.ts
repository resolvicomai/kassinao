...
export type DiscordDemoPhase = 0 | 1 | 2 | 3 | 4 | 'auto';
...
export function discordDemoPage(locale: Locale, phase: DiscordDemoPhase = 'auto'): string {
  ...
  const safePhase = phase === 'auto' ? 'auto' : String(Math.max(0, Math.min(4, phase)));
...
<style>
  * { box-sizing: border-box; }
  ...
  .voice-user.recording { color: #f2f3f5; }
  .voice-user.recording::after { content: " REC"; margin-left: 6px; color: #f23f43; font-size: 9px; font-weight: 800; }
  ...
  .chat { display: grid; grid-template-rows: 48px 1fr 68px; min-width: 0; background: #313338; }
  .chat-head {
    display: flex;
    align-items: center;
    gap: 10px;
  ...
  .message { display: none; grid-template-columns: 42px minmax(0, 1fr); gap: 12px; padding: 8px 0; }
  .avatar { display: grid; place-items: center; width: 40px; height: 40px; border-radius: 50%; background: #4e5058; font-size: 13px; font-weight: 800; }
  ...
  .embed p { margin: 6px 0 0; color: #b5bac1; font-size: 12px; line-height: 1.4; }
  .tracks { display: flex; gap: 7px; margin-top: 9px; }
  .track { padding: 4px 7px; border: 1px solid #4e5058; border-radius: 4px; color: #dbdee1; font-size: 10px; }
  .actions { display: flex; gap: 8px; margin-top: 10px; }
  .action { padding: 6px 10px; border-radius: 4px; background: #5865f2; color: white; font-size: 11px; font-weight: 700; }
  ...
  .composer { display: flex; align-items: center; margin: 8px 20px 18px; padding: 0 15px; border-radius: 8px; background: #383a40; color: #949ba4; font-size: 13px; }
  ...
  body[data-phase="auto"] .s4 { animation-delay: 4s; }
  @keyframes reveal {
...
<body data-phase="${safePhase}">
  <main class="discord" aria-label="${esc(T('Demonstração fictícia do fluxo do Kassinão dentro do Discord', 'Fictional demo of Kassinão inside Discord'))}">
    ...
    <aside class="channels">
      <div class="server-name">${esc(T('Produto & IA', 'Product & AI'))} <span>⌄</span></div>
      <div class="channel-list">
        <p class="category">${esc(T('Canais de texto', 'Text channels'))}</p>
        <div class="channel">${esc(T('geral', 'general'))}</div>
        <div class="channel active">product-sync</div>
        <p class="category" style="margin-top:20px">${esc(T('Canais de voz', 'Voice channels'))}</p>
        <div class="channel voice active">${esc(T('Produto semanal', 'Weekly product'))}</div>
        <div class="voice-user recording">Mauro</div>
        <div class="voice-user">Bea</div>
        <div class="voice-user">Rafa</div>
      </div>
      <div class="profile"><span class="profile-avatar">M</span><span class="profile-copy"><strong>Mauro</strong><span>${esc(T('Disponível', 'Online'))}</span></span></div>
    </aside>
    <section class="chat">
      <header class="chat-head"><span class="hash">#</span><strong>product-sync</strong><span>${esc(T('A call vira memória do time', 'The call becomes team memory'))}</span><span class="fixture">${esc(T('demo fictícia', 'fictional demo'))}</span></header>
      <div class="messages">
        <article class="message s0">
          <span class="avatar">M</span>
          <div><div class="meta"><strong>Mauro</strong><time>10:00</time></div><div class="command-line">${command(T('gravar', 'record'), T('Produto semanal', 'Weekly product'))}</div></div>
        </article>
        <article class="message s1">
          ...
          <div><div class="meta"><strong>Kassinão</strong><span class="bot-tag">APP</span><time>10:00</time></div>
            <div class="embed"><div class="embed-title"><span class="rec-dot"></span>${esc(T('Gravando Produto semanal', 'Recording Weekly product'))}</div><p>${esc(T('Cada pessoa está sendo gravada em uma faixa própria. O aviso fica visível para toda a call.', 'Each person is being recorded to their own track. The recording notice stays visible to everyone.'))}</p><div class="tracks"><span class="track">Mauro · track 01</span><span class="track">Bea · track 02</span><span class="track">Rafa · track 03</span></div><div class="actions"><span class="action">${esc(T('Marcar momento', 'Mark moment'))}</span><span class="action secondary">${esc(T('Parar', 'Stop'))}</span></div></div>
          </div>
        </article>
        <article class="message s2">
          <span class="avatar">B</span>
          <div><div class="meta"><strong>Bea</strong><time>10:14</time></div><div class="command-line">${command(T('nota', 'note'), T('Plano de R$ 49; proposta até sexta', '$49 plan; proposal due Friday'))}</div></div>
        </article>
        <article class="message s3">
          <span class="avatar bot">k/</span>
          <div><div class="meta"><strong>Kassinão</strong><span class="bot-tag">APP</span><time>10:47</time></div>
            <div class="embed"><div class="embed-title">${esc(T('Reunião pronta', 'Meeting ready'))}</div><p>${esc(T('Transcrição com nomes, ata, 3 decisões e 4 tarefas. A nota de Bea ficou ligada ao minuto 14.', 'Named transcript, meeting notes, 3 decisions, and 4 action items. Bea’s note is linked to minute 14.'))}</p><div class="actions"><span class="action">${esc(T('Abrir reunião', 'Open meeting'))}</span><span class="action secondary">${esc(T('Ver tarefas', 'View tasks'))}</span></div></div>
          </div>
        </article>
        <article class="message s4">
          <span class="avatar">M</span>
          <div><div class="meta"><strong>Mauro</strong><time>11:08</time></div><div class="command-line">${command(T('perguntar', 'ask'), T('o que ficou do plano de R$ 49?', 'what happened with the $49 plan?'))}</div>
            <div class="answer"><strong>Kassinão:</strong> ${esc(T('Bea vai enviar a proposta do plano de R$ 49 até sexta.', 'Bea will send the $49 plan proposal by Friday.'))} <a href="#">${esc(T('Fonte: 00:14', 'Source: 00:14'))}</a></div>
          </div>
        </article>
      </div>
      <div class="composer">${esc(T('Conversar em #product-sync', 'Message #product-sync'))}</div>
    </section>
  </main>
</body>
</html>`;
