    ...
    : opts.user
      ? `<nav class="sidebar-nav" aria-label="${pt ? 'Navegação do app' : 'App navigation'}">${
          opts.user.scope === 'full'
            ? `<a href="/app"${opts.active === 'rec' ? ' aria-current="page"' : ''}>${pt ? 'Reuniões' : 'Meetings'}</a>${config.ownerIds.includes(opts.user.id) ? `<a href="/app/operacao">${pt ? 'Operação' : 'Operations'}</a>` : ''}`
            : ''
        }${
