  ...
  <main class="shell">
    ...
    <div class="grid">
      ...
      <section class="card">
        ...
        <p>${pt ? 'A URL não é a barreira de segurança. O app exige login Discord, membership atual em um servidor permitido e a ACL da gravação. Em regra, acessam quem iniciou, quem esteve na call ou quem tem Manage Server agora. A presença breve, tardia ou sem fala também concede acesso à gravação inteira. Perder permissão no canal não revoga a presença histórica. O acesso é revalidado; falhas do Discord fecham o acesso.' : 'The URL is not the security boundary. The app requires Discord login, current membership in an allowed server, and the recording ACL. As a rule, access is available to the starter, people present in the call, or someone who currently has Manage Server. Brief, late or silent attendance also grants access to the whole recording. Losing channel permission does not revoke historical attendance. Access is revalidated; Discord failures close access.'}</p>
      </section>
      <section class="card">
