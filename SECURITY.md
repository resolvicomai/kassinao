...
Shared production also requires explicit CPU and memory limits for core, router, public, and tunnel services whose sum leaves at least 25% of physical RAM and online CPUs for neighboring workloads. The audit proves those values across the private Compose environment, sealed overlay, and Docker runtime. I/O remains a monitored operator responsibility because no safe device throttle can be sealed without a stable host device.

The router never accepts `CF-Connecting-IP` as proof by itself. It uses the rightmost valid `X-Forwarded-For` value supplied by the immediate ingress and otherwise falls back to the peer address, then strips all incoming forwarding headers before rebuilding the single trusted hop to the core/public process. It starts separate listeners bound only to `edge0` and `host0`: Cloudflare Tunnel reaches `edge0` on its isolated link, while the host loopback publish is backed by the router-only `host0` bridge. Both listeners stay unready until both binds succeed, share one upstream admission budget, and a partial bind is force-closed. Docker only materializes the published loopback port when that bridge is non-internal, so `kas-host0` is an IPv4-only NAT bridge with inter-container communication disabled and a default bind of `127.0.0.1`. Before containers start, the sealed IPv4/IPv6 `DOCKER-USER` policy allows only established return traffic from `kas-host0` and rejects every new outbound flow; `KASSINAO-HOST` separately rejects new router-to-host connections. The router never listens on either upstream link or a wildcard address. A host reverse proxy must keep `KASSINAO_HOST_PORT` loopback-only, preserve `Host`, delete `CF-Connecting-IP`, and overwrite `X-Forwarded-For` from its own connection; accepting a client-provided value breaks the rate-limit identity boundary.
...
The bundled plaintext migration accepts only the legacy installation that predates `KASSINAO_HOST_SCOPE`; modern dedicated/shared adapter roots are already mounted and require a separately audited conversion. First prove the legacy project with `validate-legacy-dedicated-installation.sh`. `remove-legacy-health-watch.sh` removes only the three exact old watchdog artifacts after runtime provenance, root-only metadata, effective systemd fragment, empty drop-ins, and byte-match checks; it does not touch Docker or neighboring workloads. Stop but do not remove the legacy containers, then run `prepare-legacy-shared-layout.sh`: it derives recordings/cache sources from the stopped core, rejects links, hardlinks, nested mounts, active recordings, neighbor mounts/binds, and `volumes-from`, and consolidates the exact four-tree layout while preserving originals. It seals the exact legacy `.env` path, filesystem identity, metadata, and hash in a root-only control directory directly under `DATA_ROOT`, outside runtime-writable binds; the plaintext release copy of that transition manifest is removed after preparation. Only then remove the legacy containers. Open a newly formatted mapper without mounting it and run `migrate-shared-storage.sh` with that same proven legacy `.env` as `KASSINAO_MIGRATION_SOURCE_APP_ENV`; a different root-owned `0600` file or any byte/identity drift is rejected. Only operational keys present in `app.env.example` are imported; host, Compose, tunnel-token, and unknown keys are excluded without printing values. The migrator repeats the neighbor gate before copying and switching, compares content and metadata, keeps `DATA_ROOT.plaintext-before-shared-luks`, and writes an encrypted `pending` marker with a 1-168 hour deadline. The preserved plaintext is a data rollback, not a turnkey operational rollback. The public operations bundle provides a sealed, resumable topology-transition primitive for exact verified bundles, but intentionally excludes organization-specific rescue configuration, credentials, host paths, and runbooks. Operational reversal exists only if the operator prepared and target-host-tested a separate root-only adapter before migration; otherwise recovery is fix-forward through the shared adapter. Never manually restart the legacy Compose stack or reinstall dedicated controls on the shared host. After independently validating app access, a real recording, and a restorable backup, stop only Kassinão, run `prepare-legacy-shared-layout.sh --purge-originals` while the consolidated data rollback still exists, and only then run `finalize-shared-migration.sh`. The purge revalidates and unlinks only the proven legacy `.env` plus the proven source contents; the finalizer refuses to destroy the data rollback until the encrypted control proves that step. These operations remove logical filesystem copies without claiming forensic erasure from SSDs, journals, snapshots, backups, or provider storage. Rotate all migrated credentials after validation because prior backups or snapshots can retain them.

Before the first public legacy-migration helper, the private runbook must already have applied and persisted the shared-host core policy. After `prepare-legacy-shared-layout.sh`, remove the legacy containers with the exact Compose command, then run `remove-legacy-dedicated-host-controls.sh` before the shared neighbor audit. That helper accepts only a completely absent set or the complete byte-matched v1.4.9 dedicated set proven by the prepared v3 transition marker. It rejects partial controls, unknown `/etc/kassinao` or runtime entries, pending legacy rollback snapshots, residual Kassinão containers, systemd drop-ins, policy drift, and neighboring-container or Docker-PID changes. It removes no foreign artifact, does not restart Docker, and preserves the legacy release, both data roots, the original `.env`, and `maintenance.lock`.
...
A successful deployment attempts to remove its operational rollback snapshot immediately. If that cleanup or the deployment fails, state and recording metadata, but not authentication state or audio tracks, may remain until the persistent host timer removes them within `KASSINAO_ROLLBACK_RETENTION_HOURS` (default 72, accepted range 1-168), even without another deployment. This snapshot is not a backup and must live on the same verified encrypted storage boundary.
...
For a normal upgrade within the same adapter, keep the previous verified bundle until Discord recording, private app, MCP (when enabled), backup, and every public/private host pass. Rollback after container replacement is manual and may invalidate sessions. This upgrade guidance does not promise operational rollback across the legacy-plaintext-to-shared migration boundary described above.
