...
The router never accepts `CF-Connecting-IP` as proof by itself. It uses the rightmost valid `X-Forwarded-For` value supplied by the immediate ingress and otherwise falls back to the peer address, then strips all incoming forwarding headers before rebuilding the single trusted hop to the core/public process. Cloudflare Tunnel supplies that chain at the isolated edge network. A host reverse proxy must keep `KASSINAO_HOST_PORT` loopback-only, preserve `Host`, delete `CF-Connecting-IP`, and overwrite `X-Forwarded-For` from its own connection; accepting a client-provided value breaks the rate-limit identity boundary.
