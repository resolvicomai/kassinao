...
The router never accepts `CF-Connecting-IP` as proof by itself. It uses the rightmost valid `X-Forwarded-For` value supplied by the immediate ingress and otherwise falls back to the peer address, then strips all incoming forwarding headers before rebuilding the single trusted hop to the core/public process. It starts separate listeners bound only to `edge0` and `host0`: Cloudflare Tunnel reaches `edge0` on its isolated link, while the host loopback publish is backed by the router-only `host0` bridge. Both listeners stay unready until both binds succeed, share one upstream admission budget, and a partial bind is force-closed. The host bridge is internal, uses IPv4 NAT only as required by Docker port publishing, isolates IPv6, and disables inter-container communication. Because a NAT bridge still has a host-side address, the sealed IPv4/IPv6 `KASSINAO-HOST` policy rejects every new router-to-host connection on `kas-host0` while allowing only established return traffic for the loopback publish. The router never listens on either upstream link or a wildcard address. A host reverse proxy must keep `KASSINAO_HOST_PORT` loopback-only, preserve `Host`, delete `CF-Connecting-IP`, and overwrite `X-Forwarded-For` from its own connection; accepting a client-provided value breaks the rate-limit identity boundary.
