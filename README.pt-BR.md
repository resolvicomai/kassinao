...
- Acesso exige login e vínculo atual com o servidor. Calls privadas ficam com participantes, quem iniciou e admins; calls públicas no início podem acompanhar a audiência atual do canal.
...
- **Controle de acesso é código, não convenção.** Toda checagem — página web, central privada `/app`, API do conector MCP — passa pela mesma função em [`src/web/access.ts`](src/web/access.ts). Se o canal era público no início, membros atuais que ainda o enxergam podem abrir. Se era restrito, só abre para quem **iniciou**, **esteve na call** (falando ou mutado) ou é admin atual. Apagar continua restrito a quem iniciou ou a admins. Não existe um caminho "por disco" que pule essa regra.
