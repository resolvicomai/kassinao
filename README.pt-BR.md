...
| `TRANSCRIBE_COMMAND` | — | Executável local + argumentos, com `{input}`/`{output}` separados (provider `command`; sem shell) |
...
No Docker, construa com Python + faster-whisper na imagem: `docker compose build --build-arg LOCAL_TRANSCRIBE=1`. O template aceita aspas para argumentos com espaço e precisa escrever em `{output}` um JSON `[{"start":s,"end":s,"text":"..."}]`. `{input}` e `{output}` devem ser argumentos separados; pipes, redirecionamentos, expansão de variável, glob e comentários são rejeitados porque o processo roda sem shell.
